...
class ChatService {
  ...
  /// Lista czatów – backend zwraca m.in. unreadCount
  static Future<List<dynamic>> fetchChats(String userId) async {
    final uri = Uri.parse('$baseUrl/api/chats?userId=$userId');
    final resp = await http.get(uri, headers: await _headers(jsonBody: false));
  ...
  /// Wiadomości w ramach jednego matcha.
  /// UWAGA: samo odpytanie tego endpointu oznacza "przeczytanie" (lastRead).
